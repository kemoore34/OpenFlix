...
class Server:
    ''' UDP server that sends packet to clients
    '''
    def __init__(self, uri, rate, max_conn, num_req_threads, req_per_thread, log_name):
        ...
        if self.rate_[0] >= 0 and self.rate_[0] < 1e-6:
            self.rate_[0] = 1e-6
...
if __name__ == '__main__':
    ...
    if (sys.argv[0].find("pydoc") == -1):
        if (len(sys.argv) < 2 or
            sys.argv[1] == "--help" or
            sys.argv[1] == "-h"):
                print(sys.argv[0]+" URL [rate] [number of packets = 20]")
                print(sys.argv[0]+"     URL: ip_address:port.  Ex. 127.0.0.1:80.")
    ...
    main(uri, rate, max_conns, num_th, num_packets, log_name)
