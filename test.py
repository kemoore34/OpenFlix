...
class HierarchicalTopo(Topo):
	...
	log_str = ''.join(['Starting HierarchicalTopo with ', 
...
class SingleServerNet(object):
	...
	if self.log: sys.stderr.write('Starting the Single-server network for Step-1\n')
...
class HierarchicalNet(object):
    global log

    def __init__(self, c=1, b=2, a=2, ctrl_addr='127.0.0.1:6633'):
	self.log = log
        self.topo = HierarchicalTopo(c, b, a)

	ctrl_args = ctrl_addr.split(':')
	ctrl_ip = ctrl_args[0]
	ctrl_port = int(ctrl_args[1]) if len(ctrl_args) > 1 else 6633
	
	if self.log: sys.stderr.write('Starting the Hierarchical network for Step-3\n')
...
if __name__ == '__main__':
    ...
    # parse options
    parser = OptionParser()
    s_str = "PA2 step to run"
    parser.add_option("-s", "--step", dest="step", type="int", default=1,
	    help=s_str, metavar="STEP")
    r_str = "Run web traffic to test your solution"
    parser.add_option("-r", "--run", action="store_true", dest="run",
	    default=False, help=r_str)
    ...
    # set of networks to run
    nets = []
    if(options.step == 1 or options.step == 0):
	nets.append(lambda: SingleServerNet(ctrl_addr=options.ctrl_addr))
    if (options.step == 2 or options.step == 0):
	nets.append(lambda: MultiServerNet(ctrl_addr=options.ctrl_addr))
    if (options.step == 3 or options.step == 0):
	nets.append(lambda: HierarchicalNet(ctrl_addr=options.ctrl_addr))
	...
	if l.strip().lower() == 'q':
            die()
	if options.run:
	    sys.stdout.write('Hit ENTER when you are ready to start the web traffic:')
	    l = sys.stdin.readline()
	    if l.strip().lower() == 'q':
                die()
	    start(mn, 20)
	if options.test:
	    time.sleep(5)
	    mn.test()
	if not (options.run or options.test):
	    CLI(mn.net)
	stop(mn)
